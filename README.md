...
| Display | `raven_output_layout_v1` (raven_shell_v1 **version 3**) + `/sys/class/backlight` | Per-screen scale, rotation and position, applied together. Needs a compositor that offers v3, and v5 for rotation (the rotation menu is greyed out on older ones); older ones get a clear message. Brightness needs the udev rule below; the page offers to install it in your terminal. |
| Storage | `lsblk -J`, `df` | |

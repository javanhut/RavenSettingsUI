...
| Display | `raven_output_layout_v1` (raven_shell_v1 **version 3**) + `/sys/class/backlight` | Per-screen scale, rotation and position, applied together, with a live preview of the arrangement you drag screens around in (they snap edge to edge, and every screen's position is sent so the layout is kept exactly as drawn), a main display picker (needs v7), and an Identify displays button (needs v6). Needs a compositor that offers v3, and v5 for rotation (the rotation menu is greyed out on older ones); older ones get a clear message. Brightness needs the udev rule below; the page offers to install it in your terminal. |
| Storage | `lsblk -J`, `df` | |
